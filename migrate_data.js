...
const report_models = require('./data_model.js');
const MongoConnection = require('./mongo_db.js');
const PostgresConnection = require('./postgres_db.js');
...
module.exports = function(complete) {
    ...
    const models = Object.getOwnPropertyNames(report_models.all_models);
    const now = moment().format('');
    const models_index = [];

    for (const rm in models) {
    ...
    const model_transform = async function(base_model, cb) {
        // Get table name
        const base_model_input = report_models.all_models[models[base_model]];
        const model_input = base_model_input.slice();
    ...
    const pgGenerate = async function(cb) {
        // create $1, $2, $3 for PG INSERT statement
        console.log('pgGenerate');
        const insert_values_array = Array.from({ length: columns.length }, (v, k) =>
            String('$' + (k + 1)),
        );
        const insert_values_string = insert_values_array.join(', ');
        pgInsertStatement =
            'INSERT into ' +
            table_name +
            ' (' +
    ...
    const pgExtract = async function(cb) {
    ...
    async function startMongoExtract(queryType, cMessage, cb) {
        ...
        if ((no_data_flag == 'yes' && queryType == 'all_data') || (no_data_flag != 'yes' && ((queryType == 'existing_data' && updated_at_flag) || (queryType == 'new_data' && created_at_flag)))) {
            ...
            while ((found === null || found == limit) && count < max) {
                ...
                for (const md in mongo_data) {
                    try {
                        const data_row = mongo_data[md];
                        console.log(md);
