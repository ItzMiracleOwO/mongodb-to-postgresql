...
module.exports = async function(complete) {
    ...
    const all_models = Object.values(report_models.all_models);
    let model_comparison = [];

    for (i = 0; i < all_models.length; i++) {
        try {
            console.log('checking table...');
            const all_cols = all_models[i].map(function(x) {
                return x.replace(/ \|.*/g, '');
            });
            const table_name = all_cols.shift();
            ...
            table_name +
            '\'';
            const res = await PostgresConnection().query(check_text);
            model_comparison = [];
            ...
            if (res.rows.length !== 0) {
                ...
                const combine_del = toRemove.join(', DROP COLUMN ');
                const del_text =
            'ALTER TABLE ' + table_name + ' DROP COLUMN ' + combine_del;
                ...
                const combine_ins = toAdd.join(', ADD COLUMN ');
                const ins_text =
            ...
            const combine_cols = all_cols.join(', ');
            // If table does not exist, create it
            if (res.rows.length === 0) {
                console.log('creating table...');
                const text = 'CREATE TABLE ' + table_name + '(' + combine_cols + ')';
                console.log(text);
